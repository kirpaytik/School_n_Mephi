...
void Game::moveEnemies()
{
	...
	for (Road* road : land->getRoads()) {
		if (Enemy* enemy = road->getEnemy()) {
			remainingEnemies.emplace_back(enemy);
		}
	}

	bool hasMoved = true;
	while (hasMoved) {
		std::vector<std::pair<Enemy*, int>> moveTargets = parallelRead(remainingEnemies);

		remainingEnemies = parallelWrite(std::move(moveTargets));

		hasMoved = !remainingEnemies.empty();
	}
}


std::vector<std::pair<Enemy*, int>> Game::parallelRead(const std::vector<Enemy*>& remainingEnemies) const
{
	std::vector<std::pair<Enemy*, int>> moveTargets;
	for (Enemy* enemy : remainingEnemies) {
		moveTargets.emplace_back(enemy, 0);
	}

	int numThreads = std::min((int)std::thread::hardware_concurrency() - 2, (int)moveTargets.size());
	if (numThreads < 1) numThreads = 1;
	int chunkSize = (moveTargets.size() + numThreads - 1) / numThreads;

	std::vector<std::thread> threads;
	for (int t = 0; t < numThreads; ++t) {
		int start = t * chunkSize;
		int end = std::min(start + chunkSize, (int)moveTargets.size());

		threads.emplace_back([this, &moveTargets, start, end]() {
			for (int i = start; i < end; ++i) {
				std::pair<Enemy*, int>& target = moveTargets[i];
				target.second = land->checkMoveDistance(target.first);
			}
		});
	}
...
std::vector<Enemy*> Game::parallelWrite(std::vector<std::pair<Enemy*, int>> moveTargets)
{
	bool hasMoved = false;
	std::vector<Enemy*> remainingEnemies;
	...
	std::vector<std::thread> threads;
	std::mutex remainingEnemiesMutex;

	for (int t = 0; t < numThreads; ++t) {
		int start = t * chunkSize;
		int end = std::min(start + chunkSize, (int)moveTargets.size());

		threads.emplace_back([this, &moveTargets, &remainingEnemies, &remainingEnemiesMutex, &hasMoved, start, end]() {
			for (int i = start; i < end; ++i) {
				std::pair<Enemy*, int>& target = moveTargets[i];

				bool isMoved = land->moveEnemy(target.first, target.second);
				bool isKilled = killIfCorpse(target.first);
				if (isMoved || isKilled) {
					hasMoved = true;
				} else {
					// so we can try to move him again
					std::lock_guard<std::mutex> lock(remainingEnemiesMutex);
					remainingEnemies.push_back(target.first);
				}
			}
		});
	}
