...
void Game::moveEnemies()
{
	...
	bool hasMoved = true;
	while (hasMoved && !enemyQueue.empty()) {
		hasMoved = false;
		int queueSize = enemyQueue.size();

		for (int i = 0; i < queueSize; ++i) {
			Enemy* enemy = enemyQueue.front();
			enemyQueue.pop();

			bool isMoved = land->moveEnemy(enemy);
			bool isKilled = killIfCorpse(enemy);

			// если двинут или умер - сможем подвинуть кого-то ещё, нужно будет продолжить while
			if (isMoved || isKilled) {
				hasMoved = true;
			}
			// остальных (не двинулся и жив) позже попробуем двинуть ещё раз (пуш в очередь)
			else {
				enemyQueue.push(enemy);
			}
		}
