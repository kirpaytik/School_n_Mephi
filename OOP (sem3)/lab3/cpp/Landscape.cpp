...
bool Landscape::moveEnemy(Enemy* enemy)
{
	if (!enemy->isAlive())
		return true;
	...
	// if mtrap was installed under his feet in between game ticks, then it needs to be processed right now
	activateTrapIfEnemy(curr_road);
	if (!enemy->isAlive())
		return true;

	int move_dist = enemy->move();
	Road* road_temp = curr_road;
	Road* next_road = curr_road;

	std::vector<DefBuild*> mtraps;
	// move step-by-step
	while (move_dist > 0) {
		...
		if (defbuild) {
			mtraps.push_back(defbuild); // save the activated mtrap until the enemy would actually be moved
		}
	...
	for (Road* road : roads) {
		...
		if (dist < min_dist1) {
			...
			if (dist <= defbuild->getRadius()) {
				target_enemy1 = road->getEnemy();
			}
		}
		if (dist < min_dist2 && !( road->getEnemy()->hasEffect(EffectType::POISON) )) {
			min_dist2 = dist;
			if (dist <= defbuild->getRadius()) {
				target_enemy2 = road->getEnemy();
			}
		}
	}
...
Enemy* Landscape::attack_furthest_from_tower(DefBuild* defbuild) const
{
	...
	for (Road* road : roads) {
		...
		if (dist > max_dist2 && dist <= defbuild->getRadius() && !( road->getEnemy()->hasEffect(EffectType::POISON) )) {
			max_dist2 = dist;
			target_enemy2 = road->getEnemy();
		}
	}
