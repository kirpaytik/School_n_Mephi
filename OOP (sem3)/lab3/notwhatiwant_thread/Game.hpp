...
public:
	...
	void moveEnemies();
	std::vector<std::pair<Enemy*, int>> parallelRead(const std::vector<Enemy*>& remainingEnemies) const;
	std::vector<Enemy*> parallelWrite(std::vector<std::pair<Enemy*, int>> moveTargets);
