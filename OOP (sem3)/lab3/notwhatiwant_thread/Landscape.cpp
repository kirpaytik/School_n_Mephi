...
// needs to be called even if moveDist == 0 cuz of activateTrapIfEnemy
bool Landscape::moveEnemy(Enemy* enemy, int move_dist)
{
	if (!enemy->isAlive())
		return false;
	...
	// check if the wanted road is still empty
	Road* last_road = curr_road->getNextRoad(move_dist);
	std::unique_lock<std::mutex> lockLast(last_road->getMutex());
	if (last_road->getEnemy())
		return false;
	...
	while (move_dist > 0) {
		...
		std::unique_lock<std::mutex> lockNext;

		 // avoid deadlock with lockLast
		if (next_road != last_road) {
			lockNext = std::move( std::unique_lock<std::mutex>(next_road->getMutex()) );
		} else {
			lockNext = std::move(lockLast);
		}

		// just move enemy closer to potenital trap
		// also moves enemy to the last road too
		if (!next_road->getEnemy()) {
			curr_road->setEnemy(nullptr);
			lockCurr.unlock();

			next_road->setEnemy(enemy);
			lockCurr = std::move(lockNext);
			curr_road = next_road;
		}

		DefBuild* mtrap = next_road->getMTrap();
		if (mtrap) {
			attack(mtrap);
			if (!enemy->isAlive())
				return true;
...
int Landscape::checkMoveDistance(Enemy* enemy) const
{
	if (!enemy->isAlive())
		return 0;

	int enemy_move = enemy->move(); // remember
	int move_dist = enemy_move; // will be edited
	int has_moved = 0;
	Road* next_road = getRoad(enemy);
	...
	while (move_dist > 0) {
		...
		// castle is reached
		if (!next_road)
			return enemy_move - move_dist;

		// only if the next road is empty, the enemy can be moved to it
		if (!next_road->getEnemy()) {
			has_moved = enemy_move - move_dist + 1; // +1 cuz enemy can be move to it
		}
