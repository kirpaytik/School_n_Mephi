...
void moveWordToHead (List *list, Item *prev2, Item *end2);
...
// Head+Last, т.к последний уже буквально незачем менять с первым,
// ведь с первым меняем, только если после встретились подобные,
...
void individualTask(List **lists, int ammLists)
{
    ...
    for (i = 0; i < ammLists; i++) {
        ...
        // пуш одиночек после групп (причём с конца) - доделать
    } // for (i = 0; i < ammLists; i++)
}
